...
// Ermittelt die gesendeten Einstellungen (checkboxes)
    ...
    if(!empty($_POST['FbLike'])){
        @$FbLikeGoToGalleryLink = (empty($_POST['FbLikeGoToGalleryLink'])) ? '': $_POST['FbLikeGoToGalleryLink'];
    }else{
        @$FbLikeGoToGalleryLink = @$FbLikeGoToGalleryLink;
    }
    ...
    $Manipulate = (@$_POST['Manipulate']) ? '1' : '0';
...
// Ermittelt die gesendeten Einstellungen (checkboxes) --- ENDE 
    ...
    $wpdb->update(
        ...
        array('ForwardAfterRegUrl' => $ForwardAfterRegUrl,'ForwardAfterRegText' => $ForwardAfterRegText,
            ...
            'RegUserUploadOnly' => $RegUserUploadOnly,'RegUserUploadOnlyText' => $RegUserUploadOnlyText,'Manipulate' => $Manipulate),
        array('GalleryID' => $id),
        array('%s','%s',
            '%d','%s',
            '%d','%s',
    ...
    //Ganz wichtig, ansonsten werden bei vielen Servern immer / (Backslashes bei Anf�hrungszeichen und aneren speziellen Sonderzeichen) hinzugef�gt
    $mConfirmContent = preg_replace('/\\\\/', '', $mConfirmContent);

    //$content = htmlentities($content, ENT_QUOTES, 'UTF-8');

    // Magic Quotes on?
    if (get_magic_quotes_gpc()) { // eingeschaltet?
        @$_POST["mConfirmConfirmationText"] = stripslashes(@$_POST["mConfirmConfirmationText"]);
        ...
        @$_POST["mConfirmHeader"] = stripslashes(@$_POST["mConfirmHeader"]);
    ...
    $mConfirmConfirmationText = sanitize_text_field(@$_POST["mConfirmConfirmationText"]);
...
/*
    // Save changes in table gallery mail

    $mGalleryContent = @$_POST["mGalleryContent"];

    //Ganz wichtig, ansonsten werden bei vielen Servern immer / (Backslashes bei Anf�hrungszeichen und aneren speziellen Sonderzeichen) hinzugef�gt
    $mGalleryContent = preg_replace('/\\\\/', '', $mGalleryContent);

    //$content = htmlentities($content, ENT_QUOTES, 'UTF-8');

    // Magic Quotes on?
    if (get_magic_quotes_gpc()) { // eingeschaltet?
        @$_POST["mGalleryAdmin"] = stripslashes(@$_POST["mGalleryAdmin"]);
        @$_POST["mGalleryReply"] = stripslashes(@$_POST["mGalleryReply"]);
        @$_POST["mGalleryCC"] = stripslashes(@$_POST["mGalleryCC"]);
        @$_POST["mGalleryBCC"] = stripslashes(@$_POST["mGalleryBCC"]);
        @$_POST["mGalleryHeader"] = stripslashes(@$_POST["mGalleryHeader"]);
        //	@$_POST["content"] = stripslashes($content);
        //	echo "<br>ja<br>";
    }
    //	stripslashes($content);
    ...
    $mGallerySendToImageOff = (@$_POST['mGallerySendToImageOff']) ? '1' : '0';
    $mGallerySendToNotConfirmedUsers = (@$_POST['mGallerySendToNotConfirmedUsers']) ? '1' : '0';


    // Escape values wordpress sql

    $mGalleryAdmin = sanitize_text_field(@$_POST["mGalleryAdmin"]);
    $mGalleryReply = sanitize_text_field(@$_POST["mGalleryReply"]);
    $mGalleryCC = sanitize_text_field(@$_POST["mGalleryCC"]);
    $mGalleryBCC = sanitize_text_field(@$_POST["mGalleryBCC"]);
    $mGalleryHeader = sanitize_text_field(@$_POST["mGalleryHeader"]);
    //$content = sanitize_text_field($content); <<< ansonten verschieden html eingaben wie <br> und andere

    // Make htmlspecialchars

    htmlentities($mGalleryAdmin);
    htmlentities($mGalleryReply);
    htmlentities($mGalleryCC);
    htmlentities($mGalleryBCC);
    htmlentities($mGalleryHeader);
    //htmlentities($content); <<< ansonten verschieden html eingaben wie <br> und andere


    $mGalleryBlacklist = sanitize_textarea_field(htmlentities(@$_POST["mGalleryBlacklist"], ENT_QUOTES, 'UTF-8'));
    ...
    $wpdb->update(
        ...
        array(
            'Admin' => "$mGalleryAdmin",'Header' => "$mGalleryHeader",'Reply' => "$mGalleryReply",'BCC' => "$mGalleryBCC",
            'CC' => "$mGalleryCC",'Content' => "$mGalleryContent",'Blacklist' => "$mGalleryBlacklist",
            'SendToImageOff' => "$mGallerySendToImageOff", 'SendToNotConfirmedUsers' => "$mGallerySendToNotConfirmedUsers"
        ),
        array('GalleryID' => $id),
        array('%s','%s','%s','%s',
            '%s','%s','%s',
    ...
    // Save changes in table gallery mail --- ENDE
*/
