...
if($_POST['cg_check']==wp_create_nonce("check") && isset($_POST['cg_Fields'])){
		...
		// Einf�gen von Werten mit Kennzeichnung durch Activation Key zur sp�teren Wiederfindung --- ENDE
            ...
            if(stripos(@$TextEmailConfirmation,@$posUrl)!==false){
                ...
                echo "Confirmation URL can't be provided. Plz contact Administrator"; die();
            }
          //  var_dump($cg_main_mail);
        //    var_dump($proOptions->RegMailSubject);
          //  var_dump($TextEmailConfirmation);
        //    var_dump($headers);
			if(!wp_mail(@$cg_main_mail, @$proOptions->RegMailSubject, @$TextEmailConfirmation, @$headers)){
			    echo "Failed sending mail, please contact administrator";die;
            }
