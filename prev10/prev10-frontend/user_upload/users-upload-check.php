...
if($_POST["cg_upload_action"]==true){
    ...
    add_action('contest_gal1ery_mail_image_activation', 'contest_gal1ery_mail_image_activation',3,3);
...
// $test->save( $files['tmp_name'][$key] );
            ...
            if ($files['size'] > 0) {
               ...
               // $idOfInsertedImage = $wpdb->insert_id;
                ...
                if (@$_POST['form_input']){
                    ...
                    foreach ($form_input as $key => $value) {
                        ...
                        if ($i==2 AND ($ft=='nf' or $ft=='ef' or $ft=='se' or $ft=='url' or $ft=='sec')){$f_input_id = $value; continue;}

                        if ($i==3 AND ($ft=='nf' or $ft=='ef' or $ft=='se' or $ft=='url' or $ft=='sec')){$field_order = $value;  continue;}

                        if ($i==4 AND ($ft=='nf' or $ft=='ef' or $ft=='se' or $ft=='url' or $ft=='sec')){
                ...
                if($ActivateUpload==1){
                    ...
                    $fbImgSrc = wp_get_attachment_image_src($wp_image_id, 'large');
                    $fbImgSrc = $fbImgSrc[0];
