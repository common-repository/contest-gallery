...
//   jQuery(this).css('display', 'hidden');
      ...
      //  var cg_comment_in_gallery = jQuery("#cg_comment_in_gallery").val();
        ...
        // Comment Frame muss als geschlossen markiert gesetzt werden,
        // falls User sich beim schließen im CommentFrame befunden hat
        // und dann zurück zum schließen drückte
        cgJsClass.slider.vars.commentFrameOpened = false;

        if(cgJsClass.slider.vars.isMobile==true && cgJsClass.slider.vars.isIOS==true){
            if(cgJsClass.slider.vars.viewportContent){


                var viewport = jQuery("meta[name='viewport']").first();
                viewport.attr('content',cgJsClass.slider.vars.viewportContent);


            }
            else{
                jQuery("meta[name='viewport']").first().remove();

             //   return false;
            }

            jQuery('body > div').css('display','');
            jQuery('body').css('background-color','');
            jQuery('#cg_slider_main_div').css('display','');
            jQuery('#mainCGdiv').css('display','block');
...
// Wichtig! Für spätere Verarbeitung des Back-Buttons und checken hashchange mobile!!!
        ...
        var closeButtonClicked = true;

        cgJsClass.slider.slide.objects.resizingGallery(closeButtonClicked);

// Drei Scroll Varianten hier als Beispiel

    //    heightLogic();

/*        jQuery( "#cg_image_id"+cgJsClass.slider.slide.objects.currentId+"" ).load(function() {
            jQuery('html, body').animate({
                scrollTop: jQuery("#cg_image_id"+cgJsClass.slider.slide.objects.currentId+"").offset().top
            }, 0);

        });*/

    //    var elmnt = document.getElementById("cg_image_id"+cgJsClass.slider.slide.objects.currentId+"");
   //     elmnt.scrollIntoView();


        location.href = "#cg_image_id"+cgJsClass.slider.slide.objects.currentId+"";

        function cgRemoveHash() {
    ...
    closeDialogs:function () {
        if(jQuery( "#cg_AlreadyRated_dialog" ).is(':visible')){
            jQuery( "#cg_AlreadyRated_dialog" ).dialog('close');
        }
        if(jQuery( "#cg_AllVotesUsed_dialog" ).is(':visible')){
            jQuery( "#cg_AllVotesUsed_dialog" ).dialog('close');
        }
        if(jQuery( "#cgIdNotExists" ).is(':visible')){
            jQuery( "#cgIdNotExists" ).hide();
        }
    }
}