...
echo "<input type='hidden' id='cgWidthOriginalImg' value='$widthOriginalImg' >";
echo "<input type='hidden' id='cgHeightOriginalImg' value='$heightOriginalImg'>";
echo "<input type='hidden' id='cg_img_rotation' value='$rThumb'>";
...
if($cgVersion>=7){
    ...
    if((@$widthScaledGalery>1400 || @$WidthGalery>1400)){
        $urlGallerySrc = $sourceOriginalImgUrl;
    }
    else{
        $urlGallerySrc = wp_get_attachment_image_src($WpUpload, 'large');
        $urlGallerySrc = $urlGallerySrc[0];
    }
}
else{
    if($WpUpload!=0 && (@$widthScaledGalery>1920 || @$WidthGalery>1920)){
		...
		@$countRating = $wpdb->get_var( "SELECT CountR FROM $tablename WHERE rowid='$rowid'" );
...
if ($sendOrder=='rating-desc' OR $sendOrder=='rating-asc') {
	...
	// 	print_r($picsSQL);
		...
		if($AllowRating==1){
            ...
            $picsSQL = $wpdb->get_results( "SELECT id FROM $tablename WHERE GalleryID='$galeryID' AND Active='1' ORDER BY (CountR+addCountR1+addCountR2+addCountR3+addCountR4+addCountR5) $order, rowid $order LIMIT $start, $step ");

		}

    if($AllowRating==2){

        $picsSQL = $wpdb->get_results( "SELECT id FROM $tablename WHERE GalleryID='$galeryID' AND Active='1' ORDER BY (CountS+addCountS) $order, rowid $order LIMIT $start, $step ");

    }

        $nextPossibleId = false;
        $priorPossibleId = false;

        $picIdChecked = false;
        $priorPicId = false;

        foreach($picsSQL as $row){

            if($picIdChecked==true){
                $nextPossibleId = $row->id;
                break;
            }

            if($row->id==$pictureID){
                $priorPossibleId = $priorPicId;
                $picIdChecked = true;
            }
		...
		// Weitergabe des Counts bei der ersten (date-desc/asc) variante. Nicht notwend
		if(!$priorPossibleId){ $arrowLeftPic =''; }
		else{//$arrowLeftPic = "<div id='cg-arrow-left-second-div'><a href='$siteURL&picture_id=$priorPossibleIdcoded&1=".$getLook."&2=".$getOrder."&3=".$getStartLeft."&4=".$getStep."#cg-begin' ><img id='cg-arrow-left-image' style='cursor: pointer;' src='$arrowPngLeft'></a></div>"; 
			$arrowLeftPic = "<div id='cg_arrow_left' class='cg_arrow_show_prev_image' >
				<a href='$siteURL&picture_id=$priorPossibleIdcoded&1=".$getLook."&2=".$getOrder."&3=".$getStartLeft."#cg-begin' >
                    <img src='$arrowPngLeft' />
		...
		if(!$nextPossibleId){ $arrowRightPic ='<div id="cg_arrow_right">&nbsp;</div>'; }
...
if($countSelectFormOutput>1){
	...
	echo "<h3><a id='cg-begin'></a></h3>";
	echo "<p class='cg_back_to_gallery'><a href='$siteURL&1=".$getLook."&2=".$getOrder."&3=".$getStart."#cg-begin' >$language_BackToGallery</a></p>";
	echo "</div>";
	...
	foreach ($selectFormOutput as $value){
			...
			if($value->Field_Type == 'text-f' or $value->Field_Type == 'email-f' or $value->Field_Type == 'select-f' or $value->Field_Type == 'url-f'){
                ...
                $select_f_field_short = $wpdb->get_var( $wpdb->prepare(
					...
					", 
					$pictureID,$f_input_id
				) );

					if(!empty($select_f_field_short)){

                        echo "<div style='margin-left: auto;margin-right: auto;display:block;'>";
                            if($value->Field_Type == 'url-f'){
                                echo "<a href='".html_entity_decode(stripslashes($select_f_field_short))."' target='_blank' class='cg_show_image_url'>
                                        <p><b>".$value->Field_Content."</b></p></a>";
                            }
                            else{
			...
			if($value->Field_Type == 'image-f'){
					...
					echo "<div id='cg_main_div' style='width: $WidthGalery;'>";
                echo "<div id='cg-arrows-div'>";

                echo $arrowLeftPic;
                echo $arrowRightPic;
                echo "</div>";
					echo "<div id='cg_img_div' style='position:relative !important;width:$WidthGalery;overflow:hidden;'>";
					echo "<img id='cg_img_gallery' class='$cgSingleImageThumbRotation' src='$urlGallerySrc' ".@$styleCenterImage.">";
					echo "</div>";
					...
					echo "<div id='rating-div' style='width: $WidthGalery;'>";
					...
					if ($FullSize==1) {
						...
						echo "<div id='cg-show-pic-full-size' ><p><a href='$fullSizeLink' target='_blank' id='cg_full_size'>$language_FullSize</a></p></div>";
				...
				//	$marginRightFB = $marginRightFB."px";
				  ...
				  //  echo 'data-show-faces="true" data-share="false" data-share="true" style="float:left;display:inline-block;font-size:12px !important;position:absolute;" ></div>';
					...
					echo "<div id='cg-start' style='position:relative;margin-left: auto;margin-right: auto;width: $WidthGalery;'>";
					echo "<h3><a id='cg-begin'></a></h3>";
					echo "<p style='font-size:21px;'><a href='$siteURL&1=".$getLook."&2=".$getOrder."&3=".$getStart."' id='cg_back_to_gallery'>$language_BackToGallery</a></p>";
					echo "</div>";
...
echo "<input type='hidden' id='cg_categoriesCheck' value='$categoriesCheck'>";
...
echo "<input type='hidden' id='cg_picture_id_coded' value='$codedPictureId'>";
